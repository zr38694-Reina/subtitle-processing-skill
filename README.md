...
| 📋 **选择题生成** | 基于课程概要自动出 5 道选择题（含参考答案），用于复习考核 | 手册指导 |
...
生成课程概要后，可让 AI 基于概要文档自动出 5 道选择题（四选一 + 参考答案），用于复习与考核。该步骤由 AI 完成，属于工作流的一部分：
...
在 Claude 中直接说「根据课程概要出选择题」或输入 `/subtitle quiz` 即可触发。完整模板见 `SKILL.md`「第四步」。
