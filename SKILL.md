...
command: subtitle
description: >-
  【/subtitle 命令】输入 /subtitle 直接执行字幕清理流水线（含课程概要生成 + 自动出选择题）：
  语气词删除 + ASR 错误修正 + 过短条目处理 + 按章节输出概要文档 + 自动根据概要出选择题。
  命令格式：/subtitle（完整流程+概要+选择题）/subtitle clean（仅清理）/subtitle quiz（仅出题）。
  也适用于任何 AI 工具处理 SRT / .md 格式的课程字幕。
  当用户提及 字幕、SRT、语音转写、课程概要、字幕清理、语气词、ASR 错误、选择题 时触发。
---
...
6. ✅ **自动生成选择题** — 基于课程概要出 5 道选择题，输出到 `选择题/` 目录
7. ✅ **输出 SRT 到桌面** — 将清理后的字幕导出为 `.srt` 格式保存到 `~/Desktop/`
8. ✅ **输出概要到桌面** — 将课程概要文档同样输出一份到 `~/Desktop/`

**使用方式：**

| 输入 | 效果 |
|------|------|
| `/subtitle` | 自动识别当前笔记或询问要处理的文件 → 执行清理流水线 + 生成概要 + **自动出选择题** |
| `/subtitle 文件名.md` | 直接处理指定文件 + 生成概要 + 自动出选择题 |
| `/subtitle clean 文件名.md` | 仅清理（不做概要生成、不出题） |
| `/subtitle quiz` | 仅基于当前课程概要生成 5 道选择题（不重新清理字幕） |
| `/subtitle quiz 文件名.md` | 根据指定课程概要文件生成选择题 |
...
6. **自动生成选择题** — 基于刚生成的概要出 5 道选择题，输出到 `选择题/` 目录（见「第四步」）
7. **导出 SRT 到桌面** — 将清理后的字幕转换为 `.srt` 格式并保存到 `~/Desktop/`
8. **导出概要到桌面** — 将课程概要 `.md` 文档也保存一份到 `~/Desktop/`

## 概述

输入：SRT 格式课程字幕（`.md` 或 `.srt` 文件）
输出：清理后的字幕 + 可选课程概要文档 + 可选选择题文档

核心流水线步骤：

```
原始字幕 → 语气词清理 → ASR 错误修正 → 过短条目处理 → 生成概要 → 自动生成选择题 → SRT + 概要到桌面 → 交付
```
...
## 第四步：根据课程概要生成选择题（默认自动执行）

整理完课程概要后**自动**基于概要文档出 5 道选择题，用于复习与考核。**出题由 AI 完成**（无需脚本），本步记录标准化流程与输出规范。

### 触发方式

| 输入 | 效果 |
|------|------|
| `/subtitle` | 完整流程 → 清理 + 概要 + **自动出选择题**（默认行为） |
| `/subtitle quiz` | 仅基于现有课程概要出题（不重新清理字幕、不重新出概要） |
| 用户说「根据课程概要出选择题 / 出几道题」 | 同上，直接执行 |

### 出题工作流

1. **读取课程概要** — 阅读 `课程大纲汇总/{课程名} {章节号} {章节标题}-课程概要.md`（或用户指定的概要文件）
2. **提炼考点** — 按章节提取核心知识点：概念定义、模型/方法对比、流程步骤、注意事项、一句话结论
3. **生成 5 道选择题**：
   - 每题为四选一（A/B/C/D），题干 + 4 个选项
   - 考点尽量覆盖不同章节，避免重复
   - 干扰项"似对实错"：取自课程内容但表述错误，而非明显离谱
   - 偏重通识理解，考点均来自课程讲解，不超纲
   - **避免考察老师的「主观观点 / 个人价值判断」**（如「老师认为…最有价值」「课程结论是…」）；优先考概念定义、模型/方法对比、流程步骤、注意事项等客观知识点。若某章仅剩老师个人观点，则换其他章节的客观考点
   - 附「参考答案」表：给出正确选项 + 简短出处/理由
4. **保存到选择题文件夹**：
   - 若课程目录下已有 `选择题/` 子目录（如 `aigc大鹏/选择题/`），放入其中
   - 无该目录时，创建 `选择题/` 文件夹后放入
5. **可选：导出到桌面** — 同样输出一份到 `~/Desktop/`
...
> 共 5 题 · 偏重通识理解 · 考点均来自老师本节课程讲解，未超纲
> 对应课程概要：[[{课程名} {章节号} {章节标题}-课程概要]]
...
*本练习基于「{课程名} {章节号} {章节标题}」课程概要整理*
```

### 命名规范

```
{课程名}/选择题/{课程名} {章节号} {章节标题}-选择题.md
```

与课程概要命名保持一致，仅后缀由 `-课程概要.md` 改为 `-选择题.md`。示例：`AI影视短片导演课 1.5 海报产品图与主视觉的生成-选择题.md`
...
- [ ] 时间轴编号连续
- [ ] 概要文档（若有）结构清晰、信息准确
- [ ] 选择题文档（默认生成）5 题齐全、答案正确、存放入 `选择题/` 目录
- [ ] 原始文件已备份（`.bak`）
