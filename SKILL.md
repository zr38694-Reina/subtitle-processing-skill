...
command: subtitle
description: >-
  【/subtitle 命令】输入 /subtitle 直接执行字幕清理流水线（含课程概要生成、选择题生成）：
  语气词删除 + ASR 错误修正 + 过短条目处理 + 按章节输出概要文档 + 根据概要出选择题。
  命令格式：/subtitle（完整流程+概要）/subtitle clean（仅清理）/subtitle quiz（出选择题）。
...
6. ✅ **生成选择题**（`/subtitle quiz`）— 基于课程概要出 5 道选择题，输出到 `选择题/` 目录
...
| `/subtitle quiz` | 根据当前课程概要生成 5 道选择题（无需重新清理字幕） |
...
6. **（可选）生成选择题** — 基于课程概要出 5 道选择题，输出到 `选择题/` 目录（见「第四步」）
...
```
原始字幕 → 语气词清理 → ASR 错误修正 → 过短条目处理 → [可选生成概要] → [可选生成选择题] → SRT + 概要到桌面 → 交付
```
...
课程概要整理完成后，可基于概要文档出 5 道选择题，用于复习与考核。**出题由 AI 完成**（无需脚本），本步记录标准化流程与输出规范。

### 触发方式

| 输入 | 效果 |
|------|------|
| `/subtitle quiz` | 基于当前课程概要（或 `<current_note>` 指向的概要）生成选择题 |
| `/subtitle quiz 文件名.md` | 根据指定课程概要文件生成选择题 |
...
- [ ] 时间轴编号连续
- [ ] 概要文档（若有）结构清晰、信息准确
- [ ] 选择题文档（若有）5 题齐全、答案正确、存放入 `选择题/` 目录
- [ ] 原始文件已备份（`.bak`）
